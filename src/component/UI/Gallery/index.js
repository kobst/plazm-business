    ...
    <>
      ...
      <GallerySection>
        <div className="galleryImage">
          <input id="myInput" onChange={(e)=> upload(e)} type="file"
            ref={(ref) => myInput = ref} style={{display: 'none'}} />
          <p onClick={(e) => myInput.click()}><img src={PlusIcon} alt="" /> Photo</p>
        </div>
        {typeof props.image!=='undefined' ?(imageValue.map((v, key) =>
          <div key={key} onClick={()=>cancelPost(v)} className="galleryImage">
            <img src={v.image} alt="" />
          </div>)): null
        }
      </GallerySection> :
