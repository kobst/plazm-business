...
const Gallery = (props) => {
  ...
  return (
    ...
    <GallerySection className="Imgcontent">
      {props.image.length!==0 ?(imageValue.map((v, key) =>
        <div key={key} onClick={()=>cancelPost(v)} className="EventsImage">
          <img className="" src={v.image} alt="" />
        </div>)): <div className="galleryImage">
          <input id="myInput" onChange={(e)=> upload(e)}
            type="file" ref={(ref) => myInput = ref} style={{display: 'none'}} />
