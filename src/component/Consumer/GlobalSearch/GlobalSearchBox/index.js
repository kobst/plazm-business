...
const GlobalSearchBox = ({ setOffset, type }) => {
  ...
  const searchFeedListData = useSelector(
    (state) => state.myFeed.searchFeedList
  );
      ...
      <GlobalSearchInputWrap isFullWidth={type === "Explore"}>
        {type === "Explore" ? (
          ...
          <input
            value={search}
            onKeyPress={searchList}
            onChange={(e) => onChangeSearch(e, null, true)}
            disabled={loader}
            placeholder={type}
          />
        )}
        {type !== "Explore" && (
          <button>
            <img src={SearchIcon} onClick={() => searchList(null, true)} />
          </button>
        )}
      </GlobalSearchInputWrap>
      {!!search.length && searchError ? (
        <ErrorDiv>{searchError}</ErrorDiv>
      ) : null}
    </>
  );
};
