...
function HexTile(props) {
  ...
  let geomShape2 = new THREE.ShapeBufferGeometry(new THREE.Shape(points));

  geomShape2 = geomShape2.applyMatrix4(
...
function PlaceMesh(props) {
  ...
  const displacedCenterHexPosition = useStore(
    (state) => state.displacedCenterHexPosition
  );
    ...
    if (meshGroup.current) {
      if (coords.current) {
        meshGroup.current.position.lerp(
          vec.set(coords.current[0], coords.current[1], coords.current[2]),
          0.05
        );
  ...
  useEffect(() => {
    ...
    if (obj) {
      ...
      const depth = Math.max(
        Math.abs(cubes.current[0]),
        Math.abs(cubes.current[1]),
        Math.abs(cubes.current[2])
      );
