...
const _OrderDistanceSeperate = (places, defaultCenter, selectedPlace) => {
  ...
  places.forEach((element, index) => {
    const distA = distance(element.businessLocation.coordinates[1],
        element.businessLocation.coordinates[0], center.lat, center.lng);
...
const assignMolecularDict = (_places, defaultCenter, selectedPlace) => {
  ...
  for (let index = 1; index < places.length; index++) {
    ...
    let closestDistance = distance(places[index].businessLocation.coordinates[1],
        places[index].businessLocation.coordinates[0], newCenter.businessLocation.coordinates[1],
        newCenter.businessLocation.coordinates[0]);
    for (let index2 = 0; index2 < index; index2++) {
      if (index !== index2) {
        const comparedPlace = places[index2];
        const comparedDistance = distance(places[index].businessLocation.coordinates[1],
            places[index].businessLocation.coordinates[0],
            places[index2].businessLocation.coordinates[1], places[index2].businessLocation.coordinates[0]);
    ...
    const angle = bearing(nearestAssignedPlace.businessLocation.coordinates[1],
        nearestAssignedPlace.businessLocation.coordinates[0], places[index].businessLocation.coordinates[1],
        places[index].businessLocation.coordinates[0]);
...
const assignHexDict = (_places, defaultCenter, selectedPlace) => {
  ...
  for (let index = 1; index < places.length; index++) {
    const element = places[index];
    const angle = bearing(newCenterPlace.businessLocation.coordinates[1],
        newCenterPlace.businessLocation.coordinates[0], element.businessLocation.coordinates[1],
    ...
    for (let slotIndex = 1; slotIndex < newXyz.length - 1; slotIndex++) {
      ...
      const position = slot.posVector;
      const coordinates = slot.coordinate;
      const newCoordinateKey = slot.coordinate[0] + '-' + slot.coordinate[1] + '-' + slot.coordinate[2];
      if (slotDict[newCoordinateKey]) {
        continue;
      }
