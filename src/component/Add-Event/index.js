...
function AddModalBox({
  ...
  useEffect(() => {
    if (typeof value !== 'undefined') {
      ...
      if (value.start && value.end) {
        if (
          new Date(value.start).getDate() < new Date(value.end).getDate() ||
          new Date(value.start).getMonth() < new Date(value.end).getMonth() ||
          new Date(value.start).getFullYear() <
            new Date(value.end).getFullYear()
        ) {
  ...
  const handleDelete = async () => {
    const response = await fetch(
      `${process.env.REACT_APP_API_URL}/api/event-delete`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
  ...
  const checkMultiDayEvent = (e, val) => {
    if (val === 'start') {
      if (end > e) {
        if (
          new Date(e).getDate() < new Date(end).getDate() ||
      ...
      if (
        new Date(start).getDate() < new Date(e).getDate() ||
    ...
    <div>
      ...
      >
        <div className="ModalHeader">
          ...
          {editValue ? <h3>Edit Event</h3> : <h3>Add New Event</h3>}
        ...
        <div className="ContentModal">
          <FormGroup>
            ...
            <Input
              type="text"
              className={titleError ? 'active' : null}
              id="title"
              value={title}
      ...
      InputLabelProps={{
          ...
          <P>
            You may upload images under the size of 2 MB each. Any dimension
